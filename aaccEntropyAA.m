function [S]= aaccEntropyAA(filein,entropyVersion,cutoff)
...
%n = size(m.IND,1); % return size of m.IND.[1]
    ...
    mChainIDresSeq(:,1) = cellstr(m.chainID);
    mChainIDresSeq(:,2) = num2cell(m.resSeq);

    [~,first,~] = uniqueRowsCA(mChainIDresSeq,[],'first');
    [~,last,~] = uniqueRowsCA(mChainIDresSeq,[],'last');
    C=mat2cell(atomContact,last-first+1,last-first+1);
    sumCells = cellfun(@(b)sum(b(:)),C,'UniformOutput',0);
    sumMat = cell2mat(sumCells);
    resContact = sumMat > 0;
    ind = aa2int(char(aminolookup(m.resName(first))));
